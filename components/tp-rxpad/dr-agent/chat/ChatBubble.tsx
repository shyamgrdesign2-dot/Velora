    ...
    <>
      <button
        ...
        className={cn(
          ...
          isOpen
            ? "text-tp-blue-600 bg-tp-blue-50"
...
export function ChatBubble({
  ...
  // ── AI row vertical alignment: center when the text is a single line, top-align once it wraps.
  //    Uses ResizeObserver so streaming text updates correctly without a re-measure on every tick.
  const aiTextRef = useRef<HTMLParagraphElement | null>(null)
  const [aiTextMultiLine, setAiTextMultiLine] = useState(false)
  useLayoutEffect(() => {
    const el = aiTextRef.current
    if (!el) return
    const measure = () => {
      const lh = parseFloat(getComputedStyle(el).lineHeight) || 18
      setAiTextMultiLine(el.offsetHeight > lh * 1.5)
    }
    measure()
  ...
  }, [message.text, displayText, shouldStream])
        ...
        <div className="flex max-w-[85%] flex-col items-end gap-[2px]">
          ...
          {message.text ? (
              <div
                ...
                style={{
                  // Quiet grey bubble — sits between slate-100 (too
                  // light, lost against the violet wash) and slate-200
                  // (too heavy). Roughly slate-150 / 94% opacity: the
                  // bubble outline is visible without competing with
                  // the assistant cards below it.
      ...
      <div className="flex w-full flex-col items-start">
        ...
        {message.text && (
          <div
            className={cn(
              "flex gap-[6px]",
              aiTextMultiLine ? "items-start" : "items-center",
            )}
          >
            {/* Bigger spark-only icon — centered with first line when single, top-aligned when wrapped */}
            <AiBrandSparkIcon
              size={22}
              className={cn("shrink-0", aiTextMultiLine && "mt-[1px]")}
            />

            {/* Plain text with streaming reveal */}
            <p
              ref={aiTextRef}
              className="text-[14px] leading-[18px] text-tp-slate-700 whitespace-pre-wrap break-words"
            >
        ...
        {!message.text && message.rxOutput && (
          <div className="flex items-start gap-[6px]">
            <AiBrandSparkIcon size={22} className="mt-[1px] shrink-0" />
          </div>
        )}

        {/* Card output -- offset to align under text (22px icon + 6px gap + 2px optical) */}
        ...
        {/* Inline AI gradient pills — only under “situation at a glance”; other replies use sticky chips above input */}
