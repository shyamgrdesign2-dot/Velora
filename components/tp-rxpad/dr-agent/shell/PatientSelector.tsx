...
interface PatientSelectorProps {
   ...
   *  When null/undefined, the legacy auto-commit-on-click behaviour is preserved. */
...
export function PatientSelector({
  ...
  const isDraftMode = !!confirmCtaLabel

  // In draft mode a row click only stages; commit happens via the CTA.
  // Legacy mode commits + closes immediately.
  ...
  const handleConfirm = useCallback(() => {
    ...
    onSelect(draftId)
    onClose?.()
  }, [draftId, onSelect, onClose])

  // Highlight follows draft selection in draft mode, committed selection otherwise.
  const visibleSelectedId = isDraftMode ? draftId : selectedId
  const isUniversalSelected = universalOptionId ? visibleSelectedId === universalOptionId : false
  ...
  return (
    <div className={cn("absolute inset-0 z-50 flex flex-col items-center justify-end", className)}>
      ...
      {/* Bottom sheet — centered + width-matched to the chat input above (max-w-[800px]
       *  with the same px-[12px] inset). Full-width on mobile, capped on desktop. */}
      <div className="relative z-10 flex w-full max-w-[800px] flex-col overflow-hidden rounded-t-[16px] bg-[#F8F9FA] mx-[12px]" style={{ height: "min(60%, 640px)", animation: "psSlideUp 200ms ease-out" }}>
        ...
        <div className="sticky top-0 z-10" style={{ background: "#F8F9FA" }}>
                ...
                {showInfoTip && (
                  <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-[6px] z-[9999] pointer-events-none">
                    <div className="rounded-[6px] bg-tp-slate-800 px-[8px] py-[5px] text-[12px] leading-[1.4] text-white shadow-lg" style={{ width: 210, whiteSpace: "normal" }}>
                      Choose which patient Velora focuses on. Selecting a patient lets you ask clinical questions, view summaries, and get insights specific to that patient.
          ...
          {/* ── Search (always at the top in draft mode) ── */}
          {isDraftMode && (
          ...
          {showUniversalOption && universalOptionId && (
            <div className="px-[8px] py-[6px]">
              <button type="button" onClick={() => handleRowClick(universalOptionId)}
          ...
          {/* "or" divider — legacy mode only (draft mode shows the search above instead) */}
          {!isDraftMode && showUniversalOption && universalOptionId && (
          ...
          {/* Search — legacy position (below Clinic Overview) */}
          {!isDraftMode && (
        ...
        {/* ── Sticky Continue CTA — only in draft mode ── */}
        {isDraftMode && (
          <div className="border-t border-tp-slate-100 px-[16px] py-[12px]" style={{ background: "#F8F9FA" }}>
            <button
              type="button"
              onClick={handleConfirm}
              disabled={!draftId}
              className={cn(
                "ps-confirm-cta flex h-[40px] w-full items-center justify-center rounded-[10px] text-[13px] font-semibold text-white transition-all",
                !draftId && "cursor-not-allowed opacity-60",
              )}
            >
              {confirmCtaLabel}
            </button>
      ...
      <style>{`
        ...
        .ps-confirm-cta{background:var(--tp-blue-500, #4B4AD5);box-shadow:0 6px 16px -8px rgba(75,74,213,0.45)}
        .ps-confirm-cta:hover:not(:disabled){background:var(--tp-blue-600, #3C3BB5)}
        .ps-confirm-cta:active:not(:disabled){transform:scale(0.99)}
      `}</style>
    </div>
  )
}
